...
def get_branch_address_data(module_name):
    return data_getter('apred', module_name, ('initialize_branch_address_predictor', 'last_branch_address_result', 'predict_branch_address'))
...
def get_ooo_cpu_module_lines(branch_address_data, branch_data, btb_data):
    branch_address_prefix = 'a'
    branch_address_varname = 'A_FLAG'
    branch_address_variant_data = [
        ('initialize_branch_address_predictor',),
        ('predict_branch_address', (('uint64_t', 'ip'),), 'uint64_t', 'std::bit_or'),
        ('last_branch_address_result', (('uint64_t', 'ip'), ('uint64_t', 'target'), ('uint8_t', 'taken'), ('uint8_t', 'branch_type')))
    ]
    ...
    classname = 'O3_CPU::module_model<' + branch_address_varname + ', ' + branch_varname + ', ' + btb_varname + '>'

    return (
        itertools.chain(
            constants_for_modules(branch_address_prefix, branch_address_data.values()), ('',),
            ...
            *(get_module_variant_declarations(fname, [v['func_map'][fname] for v in branch_address_data.values()], *finfo) for fname, *finfo in branch_address_variant_data),
        ...
        itertools.chain(
            *(get_branch_discriminator(fname, branch_address_varname, branch_varname, btb_varname, [(branch_address_prefix + v['name'], v['func_map'][fname]) for v in branch_address_data.values()], *finfo, classname=classname) for fname, *finfo in branch_address_variant_data),
            *(get_branch_discriminator(fname, branch_varname, branch_address_varname, btb_varname, [(branch_prefix + v['name'], v['func_map'][fname]) for v in branch_data.values()], *finfo, classname=classname) for fname, *finfo in branch_variant_data),
            *(get_branch_discriminator(fname, btb_varname, branch_address_varname, branch_varname, [(btb_prefix + v['name'], v['func_map'][fname]) for v in btb_data.values()], *finfo, classname=classname) for fname, *finfo in btb_variant_data)
        )
       )
