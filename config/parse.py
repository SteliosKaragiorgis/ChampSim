...
def parse_normalized(cores, caches, ptws, pmem, vmem, merged_configs, branch_context, branch_address_context, btb_context, prefetcher_context, replacement_context, compile_all_modules):
    ...
    for c in cores:
        ...
        c.setdefault('branch_address_predictor', 'bit_level_perceptron')
    ...
    cores = list(util.combine_named(cores,
            ...
            ({'name': c['name'], '_branch_address_predictor_data': [branch_address_context.find(f) for f in util.wrap_list(c.get('branch_address_predictor',[]))]} for c in cores),
    ...
    module_info = {
            ...
            'branch_address': util.combine_named(*(c['_branch_address_predictor_data'] for c in cores), branch_address_context.find_all()),
    ...
    if compile_all_modules:
        ...
        modules_to_compile = [*set(d['name'] for d in itertools.chain(
            ...
            *(c['_branch_address_predictor_data'] for c in cores),
...
def parse_config(*configs, module_dir=[], branch_dir=[], branch_address_dir=[], btb_dir=[], pref_dir=[], repl_dir=[], compile_all_modules=False):
    ...
    elements, modules_to_compile, module_info, config_file, env = parse_normalized(*normalize_config(merged_configs),
        ...
        branch_address_context = modules.ModuleSearchContext([*(os.path.join(m, 'branch_address') for m in module_dir), *branch_address_dir, os.path.join(champsim_root, 'branch_address')]),
    ...
    module_info = {
            ...
            'branch_address': {k: util.chain(v, modules.get_branch_address_data(v['name'])) for k,v in module_info['branch_address'].items()},
