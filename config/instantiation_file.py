...
def get_instantiation_lines(cores, caches, ptws, pmem, vmem):
    ...
    for cpu in cores:
        ...
        if cpu.get('_branch_address_predictor_data'):
            yield '.branch_address_predictor<{}>()'.format(' | '.join('O3_CPU::a{}'.format(k['name']) for k in cpu['_branch_address_predictor_data']))
